...
### **Funil Inteligente: Dados Estruturados + LLM Focado**
...
#### **Fase 2: Pontuação Preliminar por Relevância Temática** (~0.1s)
- **Objetivo:** Classificar empresas elegíveis por relevância temática
- **Método:** Análise de texto estruturada + expansão semântica
- **Características:**
  - **Extração de Keywords:** Título (peso 3x), descrição (peso 2x), critérios (peso 1.5x)
  - **Expansão Semântica:** Sinônimos e termos relacionados (I&D → investigação, desenvolvimento, inovação)
  - **Pontuação Normalizada:** Score baseado em frequência ponderada
- **Resultado:** ~100 → Top 20-30 candidatas com score de relevância

#### **Fase 3: Re-ranking e Análise Semântica Focada** (~15s)
- **Objetivo:** Análise semântica profunda apenas das empresas mais promissoras
- **Modelo:** Gemini 2.0 Flash com prompt otimizado
- **Eficiência:** LLM aplicado apenas a 20-30 empresas (vs 250k original)
- **Análise:** Compatibilidade semântica + justificação detalhada
- **Resultado:** Top 20-30 → **5 matches finais** com scores 8-10/10

### **Performance Otimizada**
- **Tempo Total:** ~17 segundos por incentivo (25% mais rápido)
- **Qualidade:** Scores LLM consistentemente 8-10/10
- **Eficiência:** 99% de redução no uso de LLM (250k → 30 empresas)
- **Precisão:** Filtragem inteligente garante alta relevância
- **Escalabilidade:** Processamento de 500 incentivos em ~2.5 horas
...
Projeto Augusta/
├── core/
│   ├── rag_system.py          # Sistema RAG completo (NOVO)
│   ├── optimized_matching.py  # Matching com LLM
...
├── chroma_db/                 # Vector store (auto-criado)
...
### **5. Chatbot RAG Avançado**

#### **Retrieval-Augmented Generation com IA**
...
- **Latência:** < 6s para primeira resposta

#### **Sistema Inteligente de Busca (5 Camadas)**
...
**2. Descoberta Dinâmica de Setores**
- Busca em: `cae_primary_label` + `trade_description_native` + `keywords`
- Cobertura total de setores econômicos
- Auto-descoberta de CAEs relevantes do banco

**3. Filtragem Ultra-Específica**
- Inclusão: ['robot', 'robotic', 'automation', 'cnc'] para robótica
- Exclusão: software/consultoria genérica quando buscar hardware
- Prioridade para termos em description + keywords

**4. Busca SQL Direta por CAE**
- Bypass vetorial quando CAEs identificados
- Precisão garantida por SQL direto
- Exemplo: WHERE label LIKE '%robot%' OR description LIKE '%robótica%'

**5. Respostas Contextualizadas**
- Múltiplos turnos de conversa
- 30 documentos consultados por query
